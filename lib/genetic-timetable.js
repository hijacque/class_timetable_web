...
class FacultyLoading {
    ...
    evolve(generations = 100, optimality = 0.8, limit = generations * 2) {
        ...
        let currentGen = 0;
        while (currentGen <= generations || maxFitness < optimality) {
            if (currentGen > limit) {
                break;
            }
            this.reproduce();
            ...
            const fitSolution = this.members[fitnesses.indexOf(maxFitness)];
        ...
        maxFitness = Math.max(...fitnesses);
        const fittest = mostFitSolutions[fitnesses.indexOf(maxFitness)];
...
class ClassPerms {
    ...
    fitness(reqLoad, reqHours) {
        ...
        const fitness = (this.totalPrefSubjs / totalClasses) * (loadAccuracy * 0.70 + hoursDeviation / meanHours * 0.30);
        return fitness;
    }
...
const scheduleClasses = function (classes, rooms, days, openHours, clockIn, quantum) {
    ...
    for (let r = 0; r < assignRooms.length; r++) {
        ...
        while (assignClasses.length > 0) {
            ...
            if ((start >= 720 && start < 780) || (end > 720 && end <= 780)) {
                start = 780;
                end = (classMins > quantum) ? start + quantum : start + classMins;
            }
            
            if (end > clockIn + openHours) { // cannot fit schedule
                start = clockIn;
                nextDayClasses.push({...assignClasses.splice(c, 1)[0], prevDay: d});
                d = (d < days) ? d + 1 : 1;
                
                let pair = nextDayClasses.findIndex(
                    (nc) => (nc.prevDay > days - 2 ? Math.abs(days + 1 - nc.prevDay) : nc.prevDay + 2) == d
                ) + 1;
                
                // put back next day classes in main classes array
                if (pair > 0) {
                    assignClasses.splice(c, 0, ...nextDayClasses.splice(0, pair));
                }
                
                if (d == startDay && secondLoop) {
            ...
            let classConflict = assignRooms.some(({ id, slots }) => slots.some(rs => {
                ...
                return (rs.block == block || (rs.prof && rs.prof == prof)) && timeConflict || (timeConflict && id == assignRooms[r].id);
            }));
            
            let scheduleIsPreferred = !prof ? true : assignClasses[c].pref_sched.length <= 0 ? true : 
            assignClasses[c].pref_sched.some(
                (ps) => ps.day == d && ps.start <= start && ps.start <= end && ps.end >= start && ps.end >= end
            );

            if (!classConflict && scheduleIsPreferred) {
                slots.push({ 
                ...
                if (assignClasses[c].hours > 0) {
                    ...
                    nextDayClasses.push({...preemptClass, partial: true, prevDay: d });
                } else {
