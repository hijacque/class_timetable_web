...
class ClassPerms {
    ...
    fitness(reqLoad, reqHours) {
        ...
        const fitness = loadAccuracy * 0.40 + hoursDeviation / meanHours * 0.10 + this.totalPrefSubjs / totalClasses * 0.50;
        return fitness;
    }
...
const scheduleClasses = function (classes, rooms, days, openHours, clockIn, quantum) {
    ...
    const assignRooms = shuffle(rooms);
    for (let r = 0; r < assignRooms.length; r++) {
        ...
        while (assignClasses.length > 0) {
            ...
            if (!classConflict) {
                ...
                console.log("Too many conflicts... forward 30 mins.");
