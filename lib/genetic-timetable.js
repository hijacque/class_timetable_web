...
class FacultyLoading {
    ...
    evolve(generations = 100, optimality = 0.8, limit = generations * 2) {
        ...
        while (currentGen <= generations || maxFitness < optimality) {
            ...
            currentGen++;
            console.log(currentGen, maxFitness);
        }
    ...
    for (let r = 0; r < assignRooms.length; r++) {
        ...
        while (assignClasses.length > 0) {
            ...
            if (end > clockIn + openHours) { // cannot fit schedule
                start = clockIn;
                d = (d < days) ? d + 1 : 1;
                let pairIndex = nextDayClasses.findIndex((c) => Math.min(7 - c.day + 1, c.day + 3) == d);
                assignClasses.splice(c, 0, ...nextDayClasses.splice(0, Math.max(0, pairIndex)));
            ...
            if (!classConflict && scheduleIsPreferred) {
                ...
                if (assignClasses[c].hours > 0) {
                    ...
                    nextDayClasses.push({...preemptClass, partial: true, day: d });
                } else {
