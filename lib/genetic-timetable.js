...
class FacultyLoading {
    ...
    constructor(reqLoad, reqHours, prefSubjs = [], classes = [], size = 1, mutationRate = 0.2) {
        this.members = [];
        if (classes.length <= 0) {
    ...
    evolve(generations, optimality = 1) {
...
class ClassPerms {
    ...
    fitness(reqLoad, reqHours) {
        ...
        const fitness = loadAccuracy * 0.40 + hoursDeviation / meanHours * 0.20 + this.totalPrefSubjs / totalClasses * 0.40;
        return fitness;
    }
...
const scheduleClasses = function (classes, rooms, days, openHours, clockIn, quantum) {
    ...
    for (let r = 0; r < assignRooms.length; r++) {
        ...
        const { assignClasses, leftClasses } = classes.reduce((arr, c) => {
            arr[(!c.prefRooms || c.prefRooms.some(pr => name.includes(pr)) || c.prefRooms.length <= 0) ? "assignClasses" : "leftClasses"].push(c);
            return arr;
        }, { assignClasses: [], leftClasses: [] });
        classes = leftClasses;
        const nextDayClasses = [];

        const startDay = Math.trunc(Math.random() * days);
        let c = 0, d = startDay;
        let start = clockIn;
        let end = clockIn;
        ...
        while (assignClasses.length > 0) {
            ...
            // check other rooms if block or prof is already assigned in same time
            // TODO: fix conflict conditions
            
            console.log(`\nSchedule: ${d}, ${start} - ${end}`);
            let classConflict = assignRooms.some(({ slots }) => slots.some(rs => {
                ...
                return (rs.block == block || rs.prof == prof) && timeConflict;
            }));
            console.log("Found conflict? " + classConflict);
            ...
            } else if (assignClasses.length == 1) {
