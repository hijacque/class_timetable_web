...
const sendOTP = async function (req, res, next) {
    ...
    const user = await DB.executeQuery(`select id from Users where email = "${email}"`);
    ...
    try {
        ...
        await DB.executeQuery(
            `INSERT INTO OTPs VALUES ("${user[0].id}", "${hashPin}", DATE_ADD(CURRENT_TIMESTAMP, INTERVAL 7 DAY), ` +
