...
const sendOTP = async function (req, res, next) {
    ...
    try {
        ...
        await DB.executeQuery(
            `INSERT INTO OTPs VALUES ("${req.accountID}", "${hashPin}", DATE_ADD(CURRENT_TIMESTAMP, INTERVAL 7 DAY), ` +
        ...
        await DB.executeQuery(
            ...
            `DELETE FROM Schools WHERE account_id = "${req.accountID}" AND acc_status = "pending";`
        );
...
const getOTP = async function (req, res, next) {
    ...
    if (helpInfo) {
        ...
        const otpRecord = await DB.executeQuery(
            `SELECT u.email FROM OTPs o INNER JOIN Users u ON o.account_id = u.id WHERE u.email = "${helpInfo.email}" LIMIT 1`
        );
...
const verifyOTP = async function (req, res, next) {
    ...
    if (!otp) {
        otpRecord = await DB.executeQuery(
            `SELECT o.account_id, o.pin, o.pin_salt, o.expires_on FROM OTPs o INNER JOIN Users u ON o.account_id = u.id WHERE ` +
            `u.email = "${email}" AND expires_on > CURRENT_TIMESTAMP LIMIT 1`
        );
        otp = {
            pin: otpRecord[0]["pin"],
    ...
    if (otp.pin == inputPin) {
        await DB.executeQuery(
            `DELETE FROM OTPs WHERE account_id = "${otpRecord.account_id}" AND pin = "${otp.pin}" LIMIT 1`
        );
