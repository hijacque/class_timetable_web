...
const createFaculty = async (req, res, next) => {
    ...
    const password = crypto.createHash("sha256").update(tempPassword + pass_salt).digest("base64url");

    try {
        await DB.executeQuery(
            `INSERT INTO Users (id, type, email, password, pass_salt) VALUES ("${accountID}", 3, ` +
...
const loginAccount = async function (req, res, next) {
    ...
    let account = await DB.executeQuery(
        `SELECT id, password, pass_salt, type FROM Users WHERE email = "${email}" LIMIT 1;`
    );
