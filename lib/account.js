...
const createFaculty = async (req, res, next) => {
    ...
    try {
        ...
        const openTerms = await DB.executeQuery(
            `SELECT DISTINCT t.id AS term_id, NULL AS pref_id, '${accountID}' AS faculty_id, p.deadline FROM ` +
            `Departments d INNER JOIN Colleges col ON d.college_id = col.id INNER JOIN Terms t ON ` +
            `col.school_id = t.school_id INNER JOIN Preferences p ON t.id = p.id ` +
            `WHERE d.id = '${req.params.deptID}' GROUP BY t.id, d.id`
        );
