...
const getChairData = async (req, res, next) => {
    ...
    if (task == "faculty") {
        ...
        courses.unshift({ id: "faculty", title: "Faculty" });
        ...
        const totalTerms = (terms[0].length > terms[1].length) ? terms[0].length : terms[1].length;
        for (let i = 0; i < totalTerms; i++) {
            if (terms[0][i]) {
                let openTerm = terms[0][i]["term"];
                terms[0][i]["term_ordinal"] = (openTerm == "s") ? "Summer" : `${toOrdinal(openTerm)} Semester`;
            }
            if (terms[1][i]) {
                let closedTerm = terms[1][i]["term"];
                terms[1][i]["term_ordinal"] = (closedTerm == "s") ? "Summer" : `${toOrdinal(closedTerm)} Semester`;
            }
        }
        req.taskData.openTerms = terms[0];
        req.taskData.closedTerms = terms[1];
    }
    next();
};
...
const getFacultyData = async (req, res, next) => {
    ...
    if (!req.params.task) {
        //
    } else if (req.params.task == "schedule") {
    ...
    } else if (req.params.task == "preference") {
        const [currentTerm] = await DB.executeQuery(
            `SELECT p.id as pref_id, t.year, t.term FROM Faculty f INNER JOIN Departments d ON f.dept_id = d.id INNER JOIN ` +
            `Colleges col ON d.college_id = col.id INNER JOIN Terms t ON col.school_id = t.school_id INNER JOIN ` +
            `Preferences p ON t.id = p.term_id WHERE f.id = '${user.id}' AND t.status = 'complete' ORDER BY t.year`
        );

        if (currentTerm) {
            const subjects = await DB.executeQuery(
                `SELECT DISTINCT sub.title FROM Faculty f INNER JOIN Departments d ON f.dept_id = d.id INNER JOIN ` +
                `Subjects sub ON d.college_id = sub.college_id INNER JOIN Curricula cu ON sub.id = cu.subj_id ` +
                `WHERE f.id = '${user.id}' AND cu.term = '${currentTerm.term}' ORDER BY sub.title`
            );
            currentTerm.term = (currentTerm.term == 's') ? "Summer" : toOrdinal(currentTerm.term);
            req.taskData = {
                term: currentTerm,
