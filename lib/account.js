...
const createFaculty = async (req, res, next) => {
    ...
    try {
        ...
        const openTerms = await DB.executeQuery(
            ...
            `INNER JOIN Terms t ON col.school_id = t.school_id WHERE (d.chair_id = '${user.id}' OR col.school_id = '${user.id}')`
        );
...
const getChairData = async (req, res, next) => {
    ...
    if (task == "faculty") {
        ...
        let terms = await DB.executeQuery(
            ...
            `WHERE d.chair_id = "${user.id}" GROUP BY t.id ORDER BY t.year DESC, t.term DESC`
        );
        ...
        if (req.params.current) {
            ...
            req.taskData.currentTerm = terms[0];
        }
...
const getFacultyData = async (req, res, next) => {
    ...
    if (!task) {
        ...
        await DB.executeQuery(
            `SELECT TRUNCATE((start/60 - 7) * 2 + 1, 0) AS start, TRUNCATE((end/60 - 7) * 2 + 1, 0) AS end, ` +
            `day FROM ConsultationHours WHERE faculty_id = '${user.id}' ORDER BY day`
        ).then((data) => {
            const consultHours = [];
            for (let i = 0; i <= 6; i++) {
                if (!data[i] || data[i].day != i + 1) {
                    consultHours.push({ day: toWeekDay(i + 1), unassigned: 1 });
                    continue;
                }

                data[i].day = toWeekDay(data[i].day);
                consultHours.push(data[i]);
            }
