...
const getChairData = async (req, res, next) => {
    ...
    if (task == "faculty") {
        ...
        if (tableIndex >= 1) {
            ...
            req.taskData.prev = { id: "faculty", title: department + " Faculty"  };
            req.taskData.current = courses[tableIndex];
            req.taskData.next = courses[tableIndex + 1];
        } else if (tableIndex < 0 && courses.length > 0) {
            req.taskData.current = { id: "faculty", title: department + " Faculty"  };
            req.taskData.next = courses[tableIndex + 1];
        } else {
            req.taskData.current = { id: "faculty", title: department + " Faculty"  };
        }
...
const getFacultyData = async (req, res, next) => {
    ...
    if (!task) {
        ...
        if (currentIndex >= 0) {
            ...
            currentIndex = postedTerms[0];
            return res.redirect("schedule/" + currentIndex.year + currentIndex.term);
        }
        ...
        if (termCode) {
            ...
            current = forms.find((({ status }) => status == 'PENDING'));
            return res.redirect("preference/" + current.year + current.term);
        }
        ...
        req.taskData.current = forms[current];
