...
const createAdmin = async function(req, res, next) {
    ...
    await DB.executeQuery(
        ...
        `("${accountID}", 1, "${email}", "${hashPassword}", "${passSalt}") ` +

        // User Duplicate
        `ON DUPLICATE KEY UPDATE id = "${accountID}", password = "${hashPassword}", pass_salt = "${passSalt}"; ` +

        // Insert School
        `INSERT INTO Schools VALUES ("${accountID}", "${schoolName}" , "${termType}") ` +

        // School Duplicate
        `ON DUPLICATE KEY UPDATE id = "${accountID}", name = "${schoolName}", total_terms_yearly = "${termType}";`
    );
