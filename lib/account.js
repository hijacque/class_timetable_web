...
const getAdminData = async function (req, res, next) {
    ...
    if (task == "departments") {
        ...
        const departments = await DB.executeQuery(
            `SELECT d.id, d.name, d.college_id, col.name AS college_name FROM Colleges col LEFT JOIN Departments d ON col.id = d.college_id ` +
            `WHERE col.school_id = "${user.id}" ORDER BY col.name, d.name`
        );
        ...
        if (!currentDept) {
            req.taskData.currentDept = departments[0];
        } else {
            req.taskData.currentDept = departments.find(dept => dept.name == currentDept.split("_").join(" ")) || departments[0];
        }
...
const getFacultyData = async (req, res, next) => {
    ...
    if (!req.params.task) {
        ...
        let deptSubjects = await DB.executeQuery(
            ...
            `d.chair_id = '${user.id}' AND cu.term = '${term}' ORDER BY s.title`
        );
