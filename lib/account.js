...
const getChairData = async (req, res, next) => {
    ...
    req.taskData = {};
    if (task == "faculty") {
        let department = await DB.executeQuery(`SELECT name, id from Departments WHERE chair_id = "${req.account.id}"`);
        ...
        // const totalOpenTerms = terms[0].length;
        // const totalClosedTerms = terms[1].length;
        for (let i = 0; i < totalTerms; i++) {
            ...
            if (terms[1][i]) {
                let closed = terms[1][i]["term"];
                terms[1][i]["term_ordinal"] = (closedTerm == "s") ? "Summer" : `${toOrdinal(closedTerm)} Semester`;
            }
            // let openTerm = terms[0][i]["term"];
            // let closedTerm = terms[1][i]["term"];
            // if (term == 1) {
            //     term += "<sup>st</sup> Semester";
            // } else if (term == 2) {
            //     term += "<sup>nd</sup> Semester";
            // } else if (term == 3) {
            //     term += "<sup>rd</sup> Semester";
            // } else if (term > 4) {
            //     term += "<sup>th</sup> Semester";
            // } else if (term == "s") {
            //     term = "Summer";
            // }
            // openTerms[i]["term_ordinal"] = term;
        }
        
        // let closedTerms = await DB.executeQuery(
        //     `SELECT t.id, t.year, t.term FROM Terms t INNER JOIN Schools s ON t.school_id = s.id INNER JOIN ` +
        //     `Colleges col ON s.id = col.school_id INNER JOIN Departments d ON col.id = d.college_id ` +
        //     `WHERE d.chair_id = "${user.id}" AND t.status = 0`
        // );
        // for (let i = 0; i < closedTerms.length; i++) {
        //     let term = openTerms[i]["term"];
        //     if (term == 1) {
        //         term += "<sup>st</sup> Semester";
        //     } else if (term == 2) {
        //         term += "<sup>nd</sup> Semester";
