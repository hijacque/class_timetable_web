...
const createFaculty = async (req, res, next) => {
    ...
    try {
        ...
        const openTerms = await DB.executeQuery(
            `SELECT DISTINCT t.id AS term_id, NULL AS pref_id, '${accountID}' AS faculty_id, ` +
            `CONCAT(YEAR(p.deadline), '-', MONTH(p.deadline), '-', DAY(p.deadline), ' ', DATE_FORMAT(p.deadline, ` +
            `'%H:%i:%s')) AS deadline FROM Departments d INNER JOIN Colleges col ON d.college_id = col.id INNER JOIN ` +
            `Terms t ON col.school_id = t.school_id LEFT JOIN Preferences p ON t.id = p.term_id ` +
            `WHERE d.id = '${req.params.deptID}' GROUP BY t.id, p.deadline`
        );
        ...
        console.log(error);
        await DB.executeQuery(`DELETE FROM Users WHERE id = '${accountID}'`);
