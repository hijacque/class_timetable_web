...
const loginAccount = async function (req, res, next) {
    ...
    let account = await DB.executeQuery(
        `SELECT id, password, pass_salt, name, "admin" AS type FROM Schools WHERE ` +
        `email = "${email}" AND acc_status = "open" UNION ` +
        `SELECT div_id, password, pass_salt, last_name, "chair" AS type FROM Chairpersons ` +
        `WHERE email = "${email}" AND acc_status = "open"`
    );
    ...
    if (password == account["password"]) {
        ...
        const sessionID = (accountType == "admin") ?
            crypto.createHash("sha256").update(account["id"] + account["name"]).digest("base64"):
            crypto.createHash("sha256").update(account["div_id"] + account["last_name"]).digest("base64");
        res.cookie("account", {
            type: accountType,
            email: account["email"],
            sessionID: sessionID
        }, { signed: true, maxAge: 3600000, httpOnly: true });
        req.success = { accountType: accountType };
    }
    next();
}
