...
const createFaculty = async (req, res, next) => {
    ...
    try {
        ...
        // TODO: insert preference in open term schedules
        const openTerms = await DB.executeQuery(
            `SELECT DISTINCT t.id AS term_id FROM Departments d INNER JOIN Colleges col ON d.college_id = col.id ` +
            `INNER JOIN Terms t ON col.school_id = t.school_id WHERE t.status = 'open' AND ` +
            `(d.chair_id = '${user.id}' OR col.school_id = '${user.id}')`
        );
        console.table(openTerms);

        for (const term of openTerms) {
            term.id = crypto.randomBytes(6).toString("base64url");
            term.faculty_id = accountID;
        }

        await DB.executeQuery(
            `INSERT INTO Preferences (term_id, id, faculty_id) VALUES ` +
...
const getChairData = async (req, res, next) => {
    ...
    if (task == "faculty") {
        ...
        const currentCourse = req.params.current;
        if (currentCourse && courses.length > 0) {
            req.taskData.currentCourse = courses.find(co => currentCourse == co.title.split(" ").join("_")) || courses[0];
        } else if (courses.length > 0) {
            req.taskData.currentCourse = courses[0];
        }
        ...
        let [openTerms, closedTerms] = await DB.executeQuery(
            `SELECT t.id, t.year, t.term, t.status FROM Terms t INNER JOIN Schools s ON t.school_id = s.id INNER JOIN ` +
            `Colleges col ON s.id = col.school_id INNER JOIN Departments d ON col.id = d.college_id ` +
            `WHERE d.chair_id = "${user.id}" AND t.status = 'open' ORDER BY t.year, t.term; ` +
            `SELECT t.id, t.year, t.term, t.status FROM Terms t INNER JOIN Schools s ON t.school_id = s.id INNER JOIN ` +
            `Colleges col ON s.id = col.school_id INNER JOIN Departments d ON col.id = d.college_id ` +
            `WHERE d.chair_id = "${user.id}" AND t.status != 'open' ORDER BY t.year, t.term;`
        );

        openTerms = openTerms.map(t => ({ ...t, term_ordinal: (t.term == 's') ? 'Summer' : toOrdinal(t.term) }));
        closedTerms = closedTerms.map(t => ({ ...t, term_ordinal: (t.term == 's') ? 'Summer' : toOrdinal(t.term) }));
        
        if (req.params.current) {
            req.taskData.currentTerm = openTerms.find(({year, term}) => year+term == req.params.current) || 
                closedTerms.find(({year, term}) => year+term == req.params.current)
                || openTerms[0] || closedTerms[0];
        } else {
            req.taskData.currentTerm = openTerms[0] || closedTerms[0];
        }
        console.log(req.taskData.currentTerm);
