...
const getFacultyData = async (req, res, next) => {
    ...
    if (req.taskData.constructor === Object && Object.keys(req.taskData).length > 0 ) {
        return next();
    }
    ...
    if (!req.params.task) {
        ...
        req.taskData.subjOptions = await DB.executeQuery(
            `SELECT DISTINCT s.title FROM Courses co INNER JOIN Curricula cu ON co.id = cu.course_id ` +
            `INNER JOIN Subjects s ON cu.subj_id = s.id LEFT JOIN Faculty f ON co.dept_id = f.dept_id ` +
            `LEFT JOIN Colleges col ON s.college_id = col.id WHERE f.id = '${user.id}' AND cu.term = '${term}' ` +
            `ORDER BY s.title`
        );
