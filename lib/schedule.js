...
const resetSchedule = async (req, res, next) => {
    ...
    DB.executeQuery(
        ...
        `WHERE temp.class_no > 1 AND sc.term_id = '${currentTerm.id}'` +
        (req.body.facultyID ? ` AND sc.faculty_id = '${req.body.facultyID}';` : ';') +

        // update assigned load
        `UPDATE Preferences SET assigned_load = 0 WHERE term_id = '${currentTerm.id}'` +
        (req.body.facultyID ? ` AND faculty_id = '${req.body.facultyID}';` : ';') +

        `UPDATE Schedules SET faculty_id = NULL, room_id = NULL, day = NULL, start = NULL, end = NULL, ` +
        `mode = NULL WHERE term_id = '${currentTerm.id}'` +
        (req.body.facultyID ? ` AND faculty_id = '${req.body.facultyID}';` : ';')
    );
