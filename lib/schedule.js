...
const generateSchedule = async (req, res, next) => {
    ...
    const [{ termID, semester }] = await DB.executeQuery(
        `SELECT t.id AS termID, t.term AS semester FROM Terms t INNER JOIN Colleges col ON t.school_id = col.school_id ` +
    ...
    let deptSubjects = await DB.executeQuery(
        ...
        `d.college_id = col.id AND s.college_id = col.id WHERE d.chair_id = '${user.id}' AND cu.term = '${semester}'`
    );
    ...
    let classes = await DB.executeQuery(
        ...
        `sc.term_id = '${termID}' AND sc.faculty_id IS NULL AND d.chair_id = '${user.id}' AND ` +
        `sc.subj_id IN ('${deptSubjects.join("', '")}')`
    );
    ...
    for (const prof of faculty) {
        ...
        const facultyLoader = new FacultyLoading(
            teach_load, 50, subjects.map(({ subj_id }) => subj_id),
            classes, 80
        );
    ...
    for (const classroom of classSchedule) {
        ...
        const preAssigned = slots.filter(c => c.partial == undefined);
        ...
        if (preAssigned.length > 0) {
            console.log("Old assigned classes:");
            console.table(preAssigned);
        }
        ...
        if (firstClass.length > 0) {
            query += firstClass.reduce((q, { block, subj, units, prof, day, start, end }) => {
                return q += `UPDATE Schedules SET faculty_id = '${prof}', day = ${day}, start = ${start}, ` +
                    ...
                    `SET pr.assigned_load = pr.assigned_load + ${units || 0} WHERE pr.term_id = '${termID}' ` +
...
const getFacultySched = async (req, res, next) => {
    ...
    const [deptClasses, otherClasses] = await DB.executeQuery(
        ...
        `sc.block_id = b.id LEFT JOIN Courses co ON b.course_id = co.id LEFT JOIN Colleges col ON ` +
        `s.college_id = col.id LEFT JOIN Departments d ON col.id = d.college_id AND co.dept_id = d.id WHERE ` +
        `sc.term_id = '${termID}' AND d.chair_id != '${user.id}' GROUP BY sc.block_id, sc.subj_id ` +
        `HAVING assigned_time = 0 ORDER BY b.year, b.block_no, subject;`
    );
...
const getBlockSched = async (req, res, next) => {
    ...
    let blocks = await DB.executeQuery(
        `SELECT DISTINCT b.id, co.title AS course, b.year, b.block_no FROM Schedules sc INNER JOIN Blocks b ON ` +
        `sc.block_id = b.id INNER JOIN Courses co ON b.course_id = co.id WHERE co.id = '${courseID}' ` +
        `AND sc.term_id = '${termID}' ORDER BY b.year, b.block_no`
    );
    ...
    req.data.leftClasses = await DB.executeQuery(
        ...
        `sc.faculty_id FROM Schedules sc INNER JOIN Subjects s ON sc.subj_id = s.id ` +
    ...
    console.table(req.data.classes);
...
const getBlockSchedTable = async (req, res, next) => {
    ...
    const { year, block_no, course, courseID } = req.body || req.query;
    if (year && block_no && course && courseID) {
        let schedules = await DB.executeQuery(
            `SELECT CASE WHEN s.type IS NOT NULL THEN CONCAT(s.title, ' (', s.type, ')') ELSE s.title END ` +
            `AS subject, CONCAT(f.last_name, ', ', f.first_name, ' ', f.middle_name) AS occupant, ` +
        ...
        req.workbooks.push({
            id: `${course} Class Timetable (${currentTerm.year}-${currentTerm.term})`,
            tables: [{
                id: `Block ${year}-${block_no}`,
                body: toTimeTable(schedules, 15, 7)
            }]
        });
        return next();
    }
    ...
    const blocks = await DB.executeQuery(
        ...
        `d.chair_id = '${user.id}' ORDER BY co.title, b.year, b.block_no, sc.start, sc.end`
    );
    ...
    while (blocks.length > 0) {
        ...
        // put new table sheet in workbook per block schedule
        const { year, block_no } = blocks[0];
        blockSchedules = toTimeTable(blockSchedules, 15, 7);
...
function toTimeTable(schedules, classHours, classDays) {
    const timeTable = [];
    schedules.sort((a, b) => a.start - b.start || a.day - b.day);
    let current = new Array(classDays).fill(null, 0);
    let time = 1;
    let excessSpan = new Array(classDays).fill(0, 0);
    while (time <= classHours * 2) {
        if (schedules.length <= 0 || schedules[0].start != time) {
            timeTable.push(current.slice(0));
            current.length = excessSpan.reduce((col, row) => col -= (row > 0) ? 1 : 0, classDays);
        ...
        excessSpan[day - 1] += end - start - 1;
        current[day - 1] = {
            content: `<b>${subject}</b><br><span>Prof. ${occupant}</span><br><i>${mode} ${room ? '@ ' + room : ''}</i>`,
            rowspan: end - start
        };
        schedules.shift();
    }
