...
const generateSchedule = async (req, res, next) => {
    ...
    //TODO: separate classes into major and minor
    ...
    // TODO: get consultation hours with faculty data
    ...
    //TODO: load only major classes to department's faculty
    const assignClasses = [];
    ...
    for (let i = 0; i < faculty.length; i++) {
        ...
        const facultyLoader = new FacultyLoading(teach_load, 50, subjects, majorClasses, 100);
        let { classes, fitness } = facultyLoader.evolve(50);
    ...
    // TODO: concat loaded classes with minor classes
    console.time("\nIterative classroom scheduling exec. time");
    let classSchedule = scheduleClasses(assignClasses.concat(minorClasses), rooms, 7, 14, 7, 2);
    console.timeEnd("\nIterative classroom scheduling exec. time");
...
const getFacultySched = async (req, res, next) => {
    ...
    await DB.executeQuery(
        ...
        `ORDER BY sc.day, sc.start, course, b.year, b.block_no; ` +
        ...
        `('${deptSubjects.join("', '")}') ORDER BY sc.day, sc.start, course, b.year, b.block_no;`
    ...
    console.table(deptClasses);
    console.table(otherClasses);
...
const getBlockSched = async (req, res, next) => {
    ...
    const [deptClasses, otherClasses] = await DB.executeQuery(
        ...
        `SUM(CASE WHEN sc.start IS NULL OR sc.start IS NULL THEN 0 ELSE TRUNCATE((sc.end - sc.start)/60, 1) END) ` +
        `AS assigned_time, s.req_hours, CONCAT(f.last_name, ', ', f.first_name, ' ', f.middle_name) AS faculty_name, ` +
        `sc.faculty_id, sc.subj_id FROM Schedules sc INNER JOIN Subjects s ON sc.subj_id = s.id LEFT JOIN ` +
    ...
    await DB.executeQuery(
        ...
        `ELSE 'No professor yet' END AS faculty, bu.name AS building, ` +
        ...
        `r.bldg_id = bu.id WHERE sc.mode IS NOT NULL AND sc.day IS NOT NULL AND sc.start IS NOT NULL AND ` +
        `sc.end IS NOT NULL ORDER BY sc.day, sc.start, sc.end, subject`
    ).then(data => {
