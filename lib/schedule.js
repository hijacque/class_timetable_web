...
function convertMinutesTime(time) {
    let hours = ("00" + (time / 60)).slice(-2);
    let minutes = ("00" + (time % 60)).slice(-2);
...
const getFacultySched = async (req, res, next) => {
    ...
    ).then(data => req.data.current.classes = data.map((sched) => {
        sched.start = convertMinutesTime(sched.start);
        sched.end = convertMinutesTime(sched.end);
        sched.duration = sched.end_time - sched.start_time;
        return sched;
    }));
