...
const getFacultySched = async (req, res, next) => {
    ...
    const [deptClasses, otherClasses] = await DB.executeQuery(
        ...
        `sc.term_id = '${termID}' AND d.chair_id = '${user.id}' AND sc.subj_id IN ('${deptSubjects.join("', '")}') AND (sc.faculty_id IS NULL OR ` +
        ...
        `WHERE sc.term_id = '${termID}' AND NOT sc.subj_id IN ('${deptSubjects.join("', '")}') ` +
