...
const generateSchedule = async (req, res, next) => {
    ...
    let faculty = await DB.executeQuery(
        `SELECT p.faculty_id as id, p.id AS pref_id, (f.teach_load - p.assigned_load) AS teach_load, ` + // fix query
    ...
    let classSchedule = scheduleClasses(assignClasses.concat(minorClasses), rooms, 6, 14, 7, 2);
    console.timeEnd("\nIterative classroom scheduling exec. time");
...
const getFacultySched = async (req, res, next) => {
    ...
    await DB.executeQuery(
        ...
        `ORDER BY sc.start, course, b.year, b.block_no; ` +
        ...
        `('${deptSubjects.join("', '")}') ORDER BY sc.start, course, b.year, b.block_no;`
...
const getBlockSched = async (req, res, next) => {
    ...
    await DB.executeQuery(
        ...
        `r.bldg_id = bu.id WHERE sc.mode IS NOT NULL AND sc.faculty_id IS NOT NULL ORDER BY sc.start, sc.end, subject;` +
