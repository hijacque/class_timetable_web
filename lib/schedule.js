...
const getFacultySched = async (req, res, next) => {
    ...
    req.data.current.classes = await DB.executeQuery(
        ...
        `FROM Schedules sc INNER JOIN Blocks b ON sc.block_id = b.id INNER JOIN Courses co ON b.course_id = co.id ` +
        `INNER JOIN Subjects s ON ` +
        `sc.subj_id = s.id LEFT JOIN Rooms r ON sc.room_id = r.id WHERE sc.term_id = '${termID}' AND ` +
        `sc.faculty_id = '${faculty[current].id}' AND sc.subj_id IN ('${deptSubjects.join("', '")}') ORDER BY course, b.year, b.block_no, sc.day, sc.start`
    );
    ...
    const [deptClasses, otherClasses] = await DB.executeQuery(
        ...
        `END AS subject, TRUNCATE(SUM(sc.end - sc.start)/60, 1) AS assigned_time, s.req_hours, co.title AS ` +
        `course, s.id, sc.block_id, b.year, b.block_no FROM Schedules sc INNER JOIN Blocks b ` +
        `ON sc.block_id = b.id INNER JOIN Courses co ON b.course_id = co.id INNER JOIN Departments d ` +
        `ON co.dept_id = d.id INNER JOIN Subjects s ON sc.subj_id = s.id WHERE sc.term_id = '${termID}' AND ` +
        `(sc.faculty_id IS NULL OR sc.faculty_id = '${faculty[current].id}') AND d.chair_id = '${user.id}' AND sc.subj_id IN ` +
        `('${deptSubjects.join("', '")}') GROUP BY sc.block_id, sc.subj_id ORDER BY b.year, b.block_no, subject;` +

        `SELECT CASE WHEN s.type IS NOT NULL THEN CONCAT(s.title, ' (', s.type, ')') ELSE s.title END ` +
        `AS subject, TRUNCATE(SUM(sc.end - sc.start)/60, 1) AS assigned_time, s.req_hours, co.title AS course, ` +
        `b.year, b.block_no FROM Schedules sc INNER JOIN Blocks b ON sc.block_id = b.id INNER JOIN Subjects s ON ` +
        `sc.subj_id = s.id INNER JOIN Courses co ON b.course_id = co.id INNER JOIN Departments d ON ` +
        `co.dept_id = d.id WHERE sc.term_id = '${termID}' AND sc.faculty_id IS NULL AND d.chair_id != '${user.id}' AND sc.subj_id IN ` +
        `('${deptSubjects.join("', '")}') GROUP BY sc.block_id, sc.subj_id ORDER BY b.year, b.block_no, subject;`
    );
