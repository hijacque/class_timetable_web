...
const generateSchedule = async (req, res, next) => {
    ...
    const [{termID}] = await DB.executeQuery(
...
const getFacultySched = async (req, res, next) => {
    ...
    let deptSubjects = await DB.executeQuery(
        `SELECT cu.subj_id FROM Curricula cu INNER JOIN Courses co ON cu.course_id = co.id INNER JOIN Departments d ` +
        `ON co.dept_id = d.id INNER JOIN Colleges col ON d.college_id = col.id INNER JOIN Subjects s ON ` +
        `cu.subj_id = s.id WHERE d.chair_id = '${user.id}'`
    );
