...
const generateSchedule = async (req, res, next) => {
    ...
    for (let i = 0; i < faculty.length; i++) {
        ...
        assignClasses.push(...classes.map(c => ({ ...c, prof: id, pref_sched: prefSched })));
    ...
    // concat loaded classes with minor classes
    console.time("\nIterative classroom scheduling exec. time");
    let classSchedule = scheduleClasses(
        req.params.facultyID ? assignClasses : assignClasses.concat(minorClasses), 
        rooms, 6, 14, 7, 2
    );
    console.timeEnd("\nIterative classroom scheduling exec. time");
...
const saveFacultySchedule = async (req, res, next) => {
    ...
    if (incompleteScheds.length > 0) {
        ...
        incompleteScheds = incompleteScheds.map(
            ({ prof, id }) => ({ name: prof, id: id })
        ).filter((sched, index, arr) => arr.findIndex(s => s.id == sched.id) == index);
        console.table(incompleteScheds);

        req.incompleteScheds = incompleteScheds;
    }
    ...
    `f.dept_id = d.id INNER JOIN Terms t ON pr.term_id = t.id SET pr.sched_status = 2, pr.status = (` +
    `CASE WHEN pr.status = 'pending' THEN 3 ELSE pr.status END) WHERE ` +
...
const resetSchedule = async (req, res, next) => {
    ...
    const [currentTerm] = await DB.executeQuery(
        `SELECT t.id FROM Terms t INNER JOIN Schools s On t.school_id = s.id ` +
        `INNER JOIN Colleges col ON s.id = col.school_id INNER JOIN Departments d ON col.id = d.college_id ` +
        `WHERE d.chair_id = "${user.id}" AND CONCAT(t.year, t.term) = '${termCode}' LIMIT 1`
    );
    ...
    DB.executeQuery(
        ...
        `LEFT JOIN Preferences p ON sc.faculty_id = p.faculty_id ` +
        // `LEFT JOIN Faculty f ON p.faculty_id = f.id ` +
        // `LEFT JOIN Blocks b ON sc.block_id = b.id INNER JOIN Courses co ON b.course_id = co.id LEFT JOIN ` +
        // `Departments d ON co.dept_id = d.id AND f.dept_id = d.id ` +
        `WHERE temp.class_no > 1 AND sc.term_id = '${currentTerm.id}' AND p.sched_status = 'open'` +
        // `d.chair_id = '${user.id}' AND sc.term_id = '${currentTerm.id}' AND p.sched_status = 'open'` +
        (req.body.facultyID ? ` AND sc.faculty_id = '${req.body.facultyID}';` : ';') +

        // update assigned load
        `UPDATE Preferences p SET p.assigned_load = 0 ` +
        // `INNER JOIN Departments d ON ` +
        // `f.dept_id = d.id` +
        `WHERE p.term_id = '${currentTerm.id}' AND p.sched_status = 'open'` +
        (req.body.facultyID ? ` AND p.faculty_id = '${req.body.facultyID}';` : ';') +

        `UPDATE Schedules sc SET sc.faculty_id = NULL, sc.room_id = NULL, sc.day = NULL, sc.start = NULL, ` +
        `sc.end = NULL, sc.mode = NULL WHERE sc.term_id = '${currentTerm.id}'` +
        (req.body.facultyID ? ` AND sc.faculty_id = '${req.body.facultyID}';` : ';')
    );
